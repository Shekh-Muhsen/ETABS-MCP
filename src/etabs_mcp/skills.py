...
class SkillsManager:
    ...
    def _dir_mtime(self) -> float:
        """Return the max mtime across the skills root and its immediate children."""
        try:
            mtimes = [os.path.getmtime(self._root)]
            for child in self._root.iterdir():
                if child.is_dir():
                    skill_file = child / "SKILL.md"
                    if skill_file.is_file():
                        mtimes.append(os.path.getmtime(skill_file))
            return max(mtimes)
        except OSError:
            return 0.0
    ...
    def _scan(self) -> None:
        ...
        if self._skills and current_mtime <= self._scan_mtime:
            return  # nothing changed on disk — skip re-scan
        ...
        self._scan_mtime = current_mtime
