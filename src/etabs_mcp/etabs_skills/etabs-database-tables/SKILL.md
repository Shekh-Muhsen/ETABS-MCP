...
## Table Inventory (~127 available / ~1007 total)
...
## Edit Pattern — SetTableForEditingArray (WORKS in MCP sandbox ✓)

> **CORRECTION:** `DatabaseTables.SetTableForEditingArray` **DOES WORK** via MCP sandbox.
> Previous belief that it failed was wrong. Both RS functions and load patterns have been
> created successfully. The only pitfall: if the table is empty, `GetTableForDisplayArray`
> returns empty fields — **hardcode the field tuple** in that case (see below).

### SetTableForEditingArray return values
- `ret_stage[0] == 1` = staged successfully (**1 means OK, not 0**)
- `ret_apply[0] == 0` = applied with no fatal errors
- `ret_apply[1]` = warning count (0 = clean)
- `ret_apply[3]` = log text (parse for error/warning details)

### RS Function via DatabaseTables (VERIFIED ✓)

```python
model.SetModelIsLocked(False)

RS_TABLE  = 'Functions - Response Spectrum - ASCE7-05'
RS_FIELDS = ('Name','Period','Value','SsAndS1From','Ss','S1','TL',
             'SiteClass','Fa','Fv','SDS','SD1','DampRatio','GUID')

# Read existing (preserve other functions)
raw = model.DatabaseTables.GetTableForDisplayArray(RS_TABLE, [], 'All', 0, [], 0, [])
f   = [x for x in list(raw[2]) if x is not None]
USE_F = tuple(f) if f else RS_FIELDS   # ← hardcode fallback when table is empty
nf = len(USE_F); n = raw[3]
rows = [{USE_F[j]: list(raw[4])[i*nf+j] for j in range(nf)} for i in range(n)] if nf else []

# Build header row (one row per RS function — ASCE7-05 parametric type)
hdr = {fld: None for fld in USE_F}
hdr.update({
    'Name'        : 'BNBC2020-Z2-SD',
    'SsAndS1From' : 'UserDefined',
    'Ss'          : '0.5',           # Zone 2
    'S1'          : '0.2',
    'TL'          : '4',
    'SiteClass'   : 'F',             # ← always 'F' to preserve custom Fa/Fv
    'Fa'          : '1.35',
    'Fv'          : '2.7',
    'SDS'         : str(round(2/3*1.35*0.5, 6)),
    'SD1'         : str(round(2/3*2.7*0.2, 6)),
    'Period'      : '0',
    'Value'       : str(round(0.4 * 2/3*1.35*0.5, 6)),
})

func_name = hdr['Name']
rows_out = [r for r in rows if r.get('Name') != func_name] + [hdr]
...
result = {"staged": ret_s[0], "fatal": ret_a[0], "warnings": ret_a[1]}
```

### Auto-Seismic Load Pattern via DatabaseTables (VERIFIED ✓)

```python
LP_TABLE  = 'Load Pattern Definitions - Auto Seismic - ASCE 7-05'
LP_FIELDS = ('Name','XDir','YDir','EccRatio','TopStory','BotStory','PeriodType','CtAndX',
             'SsAndS1From','Ss','S1','TL','SiteClass','Fa','Fv','SDS','SD1','R','Omega','Cd','I')

# Step 1: Add pattern shell first (type=5=seismic auto lateral)
model.LoadPatterns.Add('EQX', 5, 0.0, True)   # ret=1 if already exists — that's OK

# Step 2: Read existing LP rows
raw2 = model.DatabaseTables.GetTableForDisplayArray(LP_TABLE, [], 'All', 0, [], 0, [])
f2 = [x for x in list(raw2[2]) if x is not None]
USE_F2 = tuple(f2) if f2 else LP_FIELDS
nf2 = len(USE_F2); n2 = raw2[3]
rows2 = [{USE_F2[j]: list(raw2[4])[i*nf2+j] for j in range(nf2)} for i in range(n2)] if nf2 else []

# Step 3: Build LP row
new_lp = {fld: None for fld in USE_F2}
new_lp.update({
    'Name': 'EQX', 'XDir': 'Yes', 'YDir': 'No', 'EccRatio': '0.05',
    'TopStory': 'ROOF', 'BotStory': 'BASE',
    'PeriodType': 'Prog Calc', 'CtAndX': '0.0466, 0.9',
    'SsAndS1From': 'UserDefined', 'Ss': '0.5', 'S1': '0.2',
    'TL': '4', 'SiteClass': 'F', 'Fa': '1.35', 'Fv': '2.7',
    'SDS': '0.45', 'SD1': '0.36', 'R': '8', 'Omega': '3', 'Cd': '5.5', 'I': '1',
})

rows_out2 = [r for r in rows2 if r.get('Name') != 'EQX'] + [new_lp]
flat_out2 = tuple(r.get(fld) for r in rows_out2 for fld in USE_F2)
ret_s2 = model.DatabaseTables.SetTableForEditingArray(LP_TABLE, 0, USE_F2, len(rows_out2), flat_out2)
ret_a2 = model.DatabaseTables.ApplyEditedTables(True)
result = {"staged": ret_s2[0], "fatal": ret_a2[0], "warnings": ret_a2[1]}
```

### Known RS Function Table Names (from GetAllTables)

```python
# ETABS has one RS table per design code — 60+ tables total. Common ones:
'Functions - Response Spectrum - ASCE7-05'       # parametric (BNBC 2020 uses this)
'Functions - Response Spectrum - ASCE7-10'
'Functions - Response Spectrum - ASCE7-16'
'Functions - Response Spectrum - ASCE7-22'
'Functions - Response Spectrum - User Defined'   # custom T-Sa points
'Functions - Response Spectrum - From File'      # import from CSV
'Functions - Response Spectrum - Eurocode8-2004'
```

### Object Model Methods for Editing
...
Called after `SetTableForEditingArray`. **BOTH work in MCP sandbox ✓** (previous docs were wrong).

```python
# ApplyEditedTables(DoNotDeletePreviousResults) → (NumFatalErrors, NumWarnMsgs, NumInfoMsgs, AllMsg, ret)
# True = DoNotDeletePreviousResults (keep results if no geometry changes — use True for RS/LP edits)
ret_a = model.DatabaseTables.ApplyEditedTables(True)
fatal_errors  = ret_a[0]   # 0 = success
warn_msgs     = ret_a[1]   # warnings count
info_msgs     = ret_a[2]
log_text      = ret_a[3]   # full log string
if fatal_errors > 0:
    print("FATAL:", log_text)
```
