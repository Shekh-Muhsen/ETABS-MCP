...
# Cache the last ProgID that successfully connected — avoids iterating all
# candidates on every execute_code call.  Reset to None if it stops working.
_cached_progid: str | None = None
_cached_progid_lock = threading.Lock()


def _get_etabs_model() -> Any:
    """Return the ETABS ``SapModel`` COM object for the running instance.

    Tries the last-known working ProgID first (cached), then falls back to
    iterating ``_HELPER_PROGIDS``.  Raises ``OSError`` when no ETABS instance
    can be found.
    ...
    with _cached_progid_lock:
        cached = _cached_progid

    # Try cached ProgID first for fast path
    if cached is not None:
        try:
            helper = comtypes.client.CreateObject(cached)
            etabs_obj = helper.GetObject(_ETABS_API_PROGID)
            if etabs_obj is not None:
                return etabs_obj.SapModel
        except Exception as exc:
            last_exc = exc
            with _cached_progid_lock:
                _cached_progid = None  # invalidate — will re-scan below
...
class InstanceRegistry:
    ...
    def get_active_instances(self) -> list[ETABSInstance]:
        ...
        if t.is_alive():
            logger.warning("ETABS COM scan timed out after 10s")
